...
function Compute_Corrections_Init(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  if (do_water_correction)
    ...
    #sum_tracers_p    = Area_Weighted_Global_Mean(mesh, grid_tracers_all)
  end
...
function Compute_Corrections!(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  # error(6868)
   ...
   grav = atmo_data.grav
  if (do_water_correction) 
    # vanish the negative qv and sum these negative qv to total 
    ...
    grid_tracers_all = deepcopy(grid_tracers_n)
    sum_tracers_n    = grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, grid_tracers_all, grid_ps_n)
    # calculate the sum of negative qv at next
    total_all        = deepcopy(total)
    total_sum        = grav*Mass_Weighted_Global_Integral(vert_coord, mesh, atmo_data, total_all, grid_ps_n)
    # do qv correction factor
    tracers_ratio   = sum_tracers_p / (sum_tracers_n)
    grid_tracers_n .*= tracers_ratio
...
function Spectral_Dynamics!(mesh::Spectral_Spherical_Mesh,  vert_coord::Vert_Coordinate, 
  ...
  grid_δt  .+= grid_δQ
  ### By CJY2
  Vert_Advection!(vert_coord, grid_tracers_c, grid_Δp, grid_M_half, Δt, vert_coord.vert_advect_scheme,  grid_δtracers)
...
function Get_Topography!(grid_geopots::Array{Float64, 3})
  ...
  grid_geopots .= read_file["grid_geopots_xyzt"][:,:,1,89]
  return
end 
